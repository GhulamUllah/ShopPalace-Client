...
export default function Cart() {
  let cartlist = useSelector((state) => state.Cart.cart)?.filter((item) => item);
  ...
  for (let index = 0; index < cartlist?.length; index++) {
    ...
    shipping += Math.round(element?.product?.price * 0.1 * element?.quantity);
  }
    ...
    <>
      <Box sx={{ p: 3, display: "flex", flexDirection: "column", rowGap: 2 }}>
        ...
        <Paper elevation={2} sx={{ overflowX: "auto" }}>
          <Typography variant="h5" fontWeight={600} color={"secondary.main"} textAlign={"center"}>
            Cart Summary
          </Typography>
          <Table>
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  <TableRow>
                    <TableCell>{subtotal} PKR</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>{shipping} PKR</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>{discount} PKR</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>{subtotal + shipping - discount} PKR</TableCell>
                  </TableRow>
                </TableCell>
              </TableRow>
            </TableBody>
          ...
          <Button variant="contained" sx={{ display: "flex", my: 2, mx: "auto" }} endIcon={<ArrowForward />} onClick={handlePayment}>
            Checkout
          </Button>
        </Paper>

        <Paper elevation={2} sx={{ display: "flex", flexDirection: "column", rowGap: 2 }}>
          <Typography variant="h5" fontWeight={600} color={"secondary.main"} textAlign={"center"} my={1}>
            Cart Items
          </Typography>

          {cartlist?.length > 0 ? (
            cartlist.map((item) => (
              <Paper elevation={3} sx={{ p: 2, display: "flex", justifyContent: "space-around" }}>
                <Box sx={{ width: "200px" }}>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "space-between", maxWidth: "400px" }}>
                  <Typography variant="h7" fontWeight={600} color={"primary.main"}>
                    <b>Title: </b>
                    {item.product.name}
                  </Typography>
                  <Typography variant="h7" fontWeight={600} color={"primary.main"}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <Button variant="contained" onClick={() => dispatch(dectocart(item._id))}>
                      -
                    </Button>
                    <Button variant="outlined" disableTouchRipple sx={{ cursor: "default" }}>
                      {item.quantity}
                    </Button>
                    <Button variant="contained" onClick={() => dispatch(inctocart(item._id))}>
                      +
                    </Button>
                  </Box>
                </Box>
                <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "space-around" }}>
                  ...
                  <Button variant="outlined" endIcon={<DeleteForever />} onClick={() => dispatch(delfromcart(item._id))}>
                    Delete
                  </Button>
                </Box>
              </Paper>
            ...
            <Paper elevation={3} sx={{ height: "200px", width: "100%", display: "flex", flexDirection: "column", rowGap: 2, justifyContent: "center", alignItems: "center" }}>
